...
namespace Hurst.LogNut
{
    ...
    public class DefaultLogRecordFormatter : ILogRecordFormatter
    {
        ...
        {
            // The general format is: 2011-04-15 23:51:10.001[hostname/subjectProgram:1.0.1.3,James|7;myLogger\Warn*v] Message
            ...
            bool showLevel = (config.IsToShowLevel && (!config.IsToShowLevelOnlyForWarningsAndAbove || logRecord.Level >= LogOutputLevel.Warn));
            //CBL  Ok, so which is it -- above or below?
            //bool showLevel = (config.ContentConfig.IsToShowLevel && ((!config.ContentConfig.IsToShowLevelOnlyForWarningsAndAbove || logger.OverrideOfIsToShowLevelOnlyForWarningsAndAbove == false) || logRecord.Level >= LogOutputLevel.Warn));
