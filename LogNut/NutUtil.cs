...
namespace Hurst.LogNut
{
    ...
    public static class NutUtil
    {
            ...
            try
            {
                if (_safeFileHandle == null)
                {
                    ...
                    _safeFileHandle = FilesystemLib.CreateFileHandle( pathname,
                                                                      creationDisposition,
                                                                      BaseLib.Native.FileAccess.GenericWrite,
                                                                      BaseLib.Native.FileShare.None,
        ...
        /// <summary>
        /// Return an array of DriveInfo objects that represent every present removable drive.
        /// </summary>
        /// <returns></returns>
        ...
        public static string GetLogOutputToFlashDriveIfPresent( string subdirectoryForPlacingLogOutput )
        {
            ...
            DriveInfo[] thumbDrives = GetRemovableDrives();
        ...
        public static string CreateOutputHeader( LogRecord logRecord, LogConfig config )
        {
            ...
            {
                // Set the level to Info temporarily, so that the prefix reflects a level of only Info.
                LogOutputLevel originalLevel = logRecord.Level;
                logRecord.Level = LogOutputLevel.Info;
...
#if !NETFX_CORE
            // Try the subject-program's execution folder.
            string executionFolder = GetProgramExecutionDirectory();
            // But don't even try if it's under AppData\LocalTemp which is where unit-tests commonly execute from.
            //CBL ?
            //if (!executionFolder.Contains( @"AppData\Local\Temp" ))
            {
                bool isExecutionFolderValid = pathTester.CanWriteTo( executionFolder, out reasonForFailure );
                if (isExecutionFolderValid)
                {
                    descriptionOfPathChosen = "The executable's folder";
                    if (sbReport.Length > 0)
                    {
                        reason = sbReport.ToString();
                    }
        ...
        #region GetLogNutSupportServiceStatus
        /// <summary>
        /// Return the status of the Windows Service as a string suitable for displaying to the end-user.
        /// </summary>
        /// <returns>a string that denotes the presence of the LogNut support service, or "Not installed"</returns>
        public static string GetLogNutSupportServiceStatus()
        {
#if NETFX_CORE
            return "Unavailable";
#else
            ...
            string statusText;
            bool isInstalled = LogManager.IsLogNutOsServiceInstalled( out status );
            if (!isInstalled)
            {
        ...
        /// <remarks>
        /// This method must be implemented here, as opposed to within some other library,
        /// because otherwise the incorrect assemblies would be identified.
        /// 
        /// If you want to make this return a specific value other than the actual execution-directory
        /// (as when unit-testing), you can assign that value to the internal property <see cref="ValueToUseForSubjectProgramFolder"/>.
        /// </remarks>
        public static string GetProgramExecutionDirectory()
        {
            if (_valueToUseForSubjectProgramFolder == null)
            {
                return GetterOfExecutionFolder.GetProgramExecutionDirectory();
                ...
                //        string assemblyPathname = callingAssembly.Location;
                //        executionDirectory = FileStringLib.GetDirectoryOfPath( assemblyPathname );
                //    }
                //}
            ...
            {
                return _valueToUseForSubjectProgramFolder;
            }
        }
        #endregion

        #region ValueToUseForSubjectProgramFolder
        /// <summary>
        /// Set (or get) the value that you want the method <see cref="GetProgramExecutionDirectory"/> to return,
        /// instead of the actual execution-directory. Used for unit-tests.
        /// </summary>
        /// <remarks>
        /// For this to be useful - you need to physically create the directory first. LogNut will not
        /// try to create it.
        /// </remarks>
        public static string ValueToUseForSubjectProgramFolder
        {
            get { return _valueToUseForSubjectProgramFolder; }
            set { _valueToUseForSubjectProgramFolder = value; }
        }
        #endregion

        #region GetterOfExecutionFolder
        /// <summary>
        /// Get the object that will serve to get the folder within which the current program is executing.
        /// </summary>
        /// <returns>an object that implements the <c>IGetterOfExecutionFolder</c> interface</returns>
        public static IGetterOfExecutionFolder GetterOfExecutionFolder
        {
            get
            {
                if (_getterOfExecutionFolder == null)
                {
                    _getterOfExecutionFolder = new GetterOfExecutionFolder();
                }
                return _getterOfExecutionFolder;
            }
            set { _getterOfExecutionFolder = value; }
        }
        #endregion
#endif
...
#if !NETFX_CORE
        #region IsLogNutOsServiceInstalled
        /// <summary>
        /// Return true if the LogNutSupportService Windows Service is installed, and indicate its' status.
        /// </summary>
        /// <param name="serviceStatus">if the service is installed - this gets set to its' status.</param>
        /// <returns>true if the Windows Service is installed</returns>
        public static bool IsLogNutOsServiceInstalled( out ServiceControllerStatus serviceStatus )
        {
            bool isInstalled;
            ServiceController ctl = ServiceController.GetServices().FirstOrDefault( s => s.ServiceName == NutUtil.LogNutWindowsServiceName );
            if (ctl == null)
            {
                isInstalled = false;
        ...
        /// <param name="logLevel">if this nullable-LogOutputLevel is other than null, it dictates which LogOutputLevel to give the new log-record</param>
        ...
        public static LogRecord MagicallyCreateTestRecord( string message, LogRecord previousRecord, bool isShownAsBeingInDesignMode, LogOutputLevel? logLevel )
        {
            LogRecord newRecord;
            ...
            else // there is a previousRecord
            {
                if (logLevel == null)
                {
                    // Cycle through the possible values of LogOutputLevel..
                    switch (previousRecord.Level)
                    {
                        case LogOutputLevel.Trace:
                            level = LogOutputLevel.Debug;
                            break;
                        case LogOutputLevel.Debug:
                            level = LogOutputLevel.Info;
                            break;
                        case LogOutputLevel.Info:
                            level = LogOutputLevel.Warn;
                            break;
                        case LogOutputLevel.Warn:
                            level = LogOutputLevel.Error;
                            break;
                        case LogOutputLevel.Error:
                            level = LogOutputLevel.Fatal;
                            break;
                        default:
                            level = LogOutputLevel.Trace;
                            break;
                    }
                }
                else
                        ...
                        switch (reader.TokenType)
                        {
                            ...
                            case JsonToken.Integer:
                                if (propertyName == "Level")
                                {
                                    ...
                                    newRecord.Level = (LogOutputLevel)levelInteger;
                                }
                                break;
        ...
        public static string RenderAsXML( LogRecord logRecord, LogConfig config )
        {
            ...
            if (logRecord.Level != LogOutputLevel.Info)
            {
        ...
        #region TryParseToLogOutputLevel
        /// <summary>
        /// Given a string that is expected to contain a representation of a LogOutputLevel value,
        /// return true if it parses successfully into a LogOutputLevel and set level to that value.
        /// </summary>
        /// <param name="text">the string to try to convert to a LogOutputLevel</param>
        /// <param name="level">the resulting LogOutputLevel</param>
        /// <returns>true if the given text does represent a LogOutputLevel</returns>
        /// <remarks>
        /// This is necessary since Enum TryParse is not available in .NET 3.5 or earlier
        /// </remarks>
        public static bool TryParseToLogOutputLevel( string text, out LogOutputLevel level )
        {
            ...
            bool ok = text.TryToParseToEnum<LogOutputLevel>( out level );
            if (!ok)
            {
                WriteToConsole( "Invalid LogOutputLevel text: " + StringLib.AsString( text ) );
            }
            return ok;
        }
        #endregion
        ...
        /// <summary>
        /// This is the implementor of the IGetterOfExecutionFolder interface that this library is to use.
        /// </summary>
        private static IGetterOfExecutionFolder _getterOfExecutionFolder;
        ...
        /// <summary>
        /// If this is other than null, then it represents the value that you want the method <see cref="GetProgramExecutionDirectory"/> to return,
        /// as opposed to the actual execution-directory. Used for unit-tests.
        /// </summary>
        private static string _valueToUseForSubjectProgramFolder;
