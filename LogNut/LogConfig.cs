                        ...
                        if (!line.StartsWith( "//" ))
                        {
                            ...
                            switch (propertyName)
                            {
                                ...
                                case "LowestLevelThatIsEnabled":
                                    ...
                                    this.LowestLevelThatIsEnabled = (LogOutputLevel)Enum.Parse( typeof( LogOutputLevel ), propertyText.PutIntoTypeOfCasing( StringLib.TypeOfCasing.Titlecased ) );
                                    break;
                                case "MaxFileSize":
                                ...
                                case "MaxNumberOfFileRollovers":
                                    ...
                                    break;
#if !NETFX_CORE
                                case "IsRegistry32Bit":
                                    this.IsRegistry32Bit = Boolean.Parse( propertyText );
                                    break;
                                case "IsRegistryHiveCurrentUser":
                                    this.IsRegistryHiveCurrentUser = Boolean.Parse( propertyText );
                                    break;
#endif
...
//newLogConfig = await Hurst.BaseLib.XmlIO.ReadObjectFromXmlFileAsync<LogConfig>( pathnameOfConfigurationFile );
        ...
        #region ReadFromRegistry
        /// <summary>
        /// Retrieve the values from the Windows Registry
        /// to use as the actual values for the configuration settings.
        /// </summary>
        /// <remarks>
        /// This is not available except with code running on the .NET Framework.
        /// Universal Windows Platform does not provide for this.
        /// </remarks>
        public void ReadFromRegistry()
        {
            var reg = LogNutRegistryService.The;

            LogOutputLevel? nullableLevel = reg.GetLowestLevelThatIsEnabled(null, null);
            if (nullableLevel.HasValue)
            {
                _lowestLevelThatIsEnabled = nullableLevel.Value;
            }

            // IsLoggingEnabled
            bool? nullableBoolValueFromRegistry = reg.GetIsLoggingEnabled(null, null);
            if (nullableBoolValueFromRegistry.HasValue)
            {
                IsLoggingEnabled = nullableBoolValueFromRegistry.Value;
            }

            // FileOutputFolder
            string stringValue = reg.FileOutputFolder;
            if (stringValue != null)
            {
                _fileOutputFolder = stringValue;
            }

            // FileOutputRolloverMode
            RolloverMode? nullableRolloverModeValueFromRegistry = reg.GetFileRolloverMode(null, null);
            if (nullableRolloverModeValueFromRegistry.HasValue)
            {
                _fileOutputRolloverMode = nullableRolloverModeValueFromRegistry.Value;
            }

            // FileOutputRollPoint
            RollPoint? nullableRollPointValueFromRegistry = reg.GetFileTimeRollPoint(null, null);
            if (nullableRollPointValueFromRegistry.HasValue)
            {
                _fileOutputRollPoint = nullableRollPointValueFromRegistry.Value;
            }

            // IsAppendingToFile
            nullableBoolValueFromRegistry = reg.GetIsToCreateNewOutputFileUponStartup( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                IsToCreateNewOutputFileUponStartup = nullableBoolValueFromRegistry.Value;
            }

            // IsFileOutputToCompressFiles
            nullableBoolValueFromRegistry = reg.GetIsCompressingOutputFiles( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isFileOutputToCompressFiles = nullableBoolValueFromRegistry.Value;
            }

            // IsFileOutputToInsertHeader
            nullableBoolValueFromRegistry = reg.GetIsInsertingBannerText( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                IsFileOutputToInsertHeader = nullableBoolValueFromRegistry.Value;
            }

            // IsFileOutputToInsertLineBetweenTraces
            nullableBoolValueFromRegistry = reg.GetIsInsertingLineBetweenTraces( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                IsFileOutputToInsertLineBetweenTraces = nullableBoolValueFromRegistry.Value;
            }

            // MaxFileSize
            Int64? nullableInt64ValueFromRegistry = reg.GetMaxFileSize(null, null);
            if (nullableInt64ValueFromRegistry.HasValue)
            {
                _maxFileSize = nullableInt64ValueFromRegistry.Value;
            }

            // MaxNumberOfFileRollovers
            int? nullableIntValueFromRegistry = reg.GetMaxNumberOfFileRollovers(null, null);
            if (nullableIntValueFromRegistry.HasValue)
            {
                _maxNumberOfFileRollovers = nullableIntValueFromRegistry.Value;
            }
            // Call upon the other ConfigurationBase-derived objects to pull in their own values..
            //CBL  ContentConfig.ReadFromRegistry();
            // IsToShowFractionsOfASecond
            nullableBoolValueFromRegistry = reg.GetIsToShowFractionsOfASecond( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowFractionsOfASecond = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowLevel
            nullableBoolValueFromRegistry = reg.GetIsToShowLevel( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowLevel = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowLoggerName
            nullableBoolValueFromRegistry = reg.GetIsToShowLoggerName( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowLoggerName = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowPrefix
            nullableBoolValueFromRegistry = reg.GetIsToShowPrefix( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowPrefix = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowSourceHost
            nullableBoolValueFromRegistry = reg.GetIsToShowSourceHost( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowSourceHost = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowStackTraceForExceptions
            nullableBoolValueFromRegistry = reg.GetIsToShowStackTraceForExceptions( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowStackTraceForExceptions = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowSubjectProgram
            nullableBoolValueFromRegistry = reg.GetIsToShowSubjectProgram( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowSubjectProgram = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowSubjectProgramVersion
            nullableBoolValueFromRegistry = reg.GetIsToShowSubjectProgramVersion( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowSubjectProgramVersion = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowThread
            nullableBoolValueFromRegistry = reg.GetIsToShowThread( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowThread = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowTimestamp
            nullableBoolValueFromRegistry = reg.GetIsToShowTimestamp( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowTimestamp = nullableBoolValueFromRegistry.Value;
            }

            // IsToShowUser
            nullableBoolValueFromRegistry = reg.GetIsToShowUser( null, null );
            if (nullableBoolValueFromRegistry.HasValue)
            {
                _isToShowUser = nullableBoolValueFromRegistry.Value;
            }
            ...
            if (LogManager._outputPipes != null)
            {
                ...
                {
                    outputPipe.ReadSettingsFromRegistry();
                }
            }
        }
        #endregion ReadFromRegistry

        #region WriteToRegistry
        /// <summary>
        ...
        {
            var reg = LogNutRegistryService.The;
            reg.SetLowestLevelThatIsEnabled( _lowestLevelThatIsEnabled, null, null );
            reg.SetIsLoggingEnabled( IsLoggingEnabled, null, null );

            reg.SetFileOutputFolder( _fileOutputFolder, null, null );
            reg.SetFileRolloverMode( FileOutputRolloverMode, null, null );
            reg.SetFileTimeRollPoint( FileOutputRollPoint, null, null );
            reg.SetIsToCreateNewOutputFileUponStartup( IsToCreateNewOutputFileUponStartup, null, null );
            reg.SetIsFileOutputEnabled( IsFileOutputEnabled, null, null );
            reg.SetIsInsertingBannerText( IsFileOutputToInsertHeader, null, null );
            reg.SetIsInsertingLineBetweenTraces( IsFileOutputToInsertLineBetweenTraces, null, null );
            reg.SetMaxFileSize( MaxFileSize, null, null );
            reg.SetMaxNumberOfFileRollovers( MaxNumberOfFileRollovers, null, null );
            //reg.FileOutputFormatType = ExplicitValue_FileOutputFormatType;

            reg.SetIsToShowFractionsOfASecond( IsToShowFractionsOfASecond, null, null );
            reg.SetIsToShowLevel( IsToShowLevel, null, null );
            reg.SetIsToShowLoggerName( IsToShowLoggerName, null, null );
            reg.SetIsToShowPrefix( IsToShowPrefix, null, null );
            reg.SetIsToShowSourceHost( IsToShowSourceHost, null, null );
            reg.SetIsToShowStackTraceForExceptions( IsToShowStackTraceForExceptions, null, null );
            reg.SetIsToShowSubjectProgram( IsToShowSubjectProgram, null, null );
            reg.SetIsToShowSubjectProgramVersion( IsToShowSubjectProgramVersion, null, null );
            reg.SetIsToShowThread( IsToShowThread, null, null );
            reg.SetIsToShowTimestamp( IsToShowTimestamp, null, null );
            reg.SetIsToShowUser( IsToShowUser, null, null );



            //CBL But this should be only for Registry?
            if (LogManager._outputPipes != null)
            {
                foreach (var pipe in LogManager.OutputPipes)
                {
                    pipe.WriteSettingsToRegistry();
                }
            }
        }
        #endregion
        ...
        public string Username
        {
            get
            {
                ...
                if (_userName == null)
                {
                    ...
                    if (this.SubjectProgram != null)
                    {
                        _userName = this.SubjectProgram.Username;
                    }
                    else
                    {
        ...
        /// <summary>
        /// Get or set the subject-program application object that implements the <see cref="IApp"/> interface, if applicable.
        /// This is null if the subject-program does not implement the <see cref="IApp"/> interface and this has not been set.
        /// </summary>
        /// <remarks>
        /// <para>If the executing application is not an <see cref="IApp"/>,
        /// then SubjectProgramName should be set explicitly.</para>
        /// 
        /// <para>This property is NOT saved in configuration files.</para>
        /// </remarks>
        [XmlIgnore]
        public IApp SubjectProgram { get; set; }

        /// <summary>
        /// Set the subject-program application object that implements the <see cref="IApp"/> interface, if applicable.
        /// This is the same as setting the <see cref="SubjectProgram"/> property.
        /// </summary>
        /// <param name="app">an instance of an <see cref="IApp"/> that represents the subject-application that is doing the logging</param>
        /// <returns>a reference to this LogConfig object so that methods may be chained together</returns>
        public LogConfig SetSubjectProgram( IApp app )
        {
            this.SubjectProgram = app;
            return this;
        }
        #endregion
        ...
        public string GetSubjectProgramName( Type typeFromApplication )
        {
            if (_subjectProgramName == null)
            {
                if (SubjectProgram == null)
                {
                    ...
                    if (_subjectProgramName.Equals( "LogNut_Net" ))
                    {
                ...
                {
                    _subjectProgramName = this.SubjectProgram.ProgramName;
                }
            }
        ...
        public string SubjectProgramVersion
        {
            get
            {
                ...
                if (_subjectProgramVersion == null)
                {
                    if (this.SubjectProgram != null)
                    {
                        _subjectProgramVersion = this.SubjectProgram.ProgramVersionText;
                    }
                    else
                    {
        ...
        //public EmailOutputPipe EmailOutput
        //{
        //    //CBL Does it make sense to even have this? Perhaps all IOutputPipes should be treated the same. ?
        //    get
        //    {
        //        EmailOutputPipe pipe = null;

        //        if (LogManager._outputPipes != null)
        //        {
        //            if (_outputPipes.Count > 0)
        //            {
        //                pipe = (from x in _outputPipes
        //                        where x.Name.Equals( "Email" )
        //                        select x) as EmailOutputPipe;

        //                // pipe = _outputPipes.Select( x => x.Name.Equals( "Email" ) ).First<EmailOutputPipe>();
        //                //if (emailPipe != null)
        //                //{
        //                //    isAnEmailPipe = true;
        //                //}
        ...
        #region Windows Registry related properties
#if !NETFX_CORE
        #region IsRegistry32Bit
        /// <summary>
        /// Get or set whether to access the 32-bit view of the Windows Registry instead of the normal 64-bit view.
        /// This only applies when running on a 64-bit Windows host. The default is true.
        /// </summary>
        /// <remarks>
        /// In order to have a consistent standard for 32-bit and 64-bit applications to interoperate, this value is presumed to be true.
        /// </remarks>
#if !SILVERLIGHT
        [DefaultValue( true )]
        public bool IsRegistry32Bit
        {
            get { return LogNutRegistryService.The.IsRegistry32Bit; }
            set { LogNutRegistryService.The.IsRegistry32Bit = value; }
        }
#endif

        /// <summary>
        /// Set to use the 32-bit view of the Windows Registry instead of the normal 64-bit view. This is the default behavior.
        /// </summary>
        /// <returns>a reference to this LogConfig object, such that further method calls may be chained</returns>
        /// <remarks>
        /// This only applies when running on a 64-bit Windows host.
        /// Calling this method is the same as setting the IsRegistry32Bit property to true.
        /// </remarks>
        public LogConfig UseThe32BitRegistry()
        {
            IsRegistry32Bit = true;
            return this;
        }

        /// <summary>
        /// Set to use (on x64 systems) the normal 64-bit view of the Windows Registry instead of the 32-bit view.
        /// To use the 32-bit view is the default.
        /// </summary>
        /// <returns>a reference to this LogConfig object, such that further method calls may be chained</returns>
        /// <remarks>
        /// This only applies when running on a 64-bit Windows host.
        /// Calling this method is the same as setting the IsRegistry32Bit property to false.
        /// </remarks>
        public LogConfig UseThe64BitRegistry()
        {
            IsRegistry32Bit = false;
            return this;
        }
        #endregion

        #region IsRegistryHiveCurrentUser
        /// <summary>
        /// Get or set whether the registry values for LogNut are accessed
        /// from that section of the Registry that is specific to the current user
        /// (ie, HKEY_CURRENT_USER) versus accessible across all users on the local box
        /// (ie, HKEY_LOCAL_MACHINE).
        /// </summary>
        /// <remarks>
        /// If you set this, you need to do it before the first Registry access if it is to be other than the default.
        /// There can be security issues related to changing values in HKEY_LOCAL_MACHINE, thus HKEY_CURRENT_USER may be the desired choice.
        /// </remarks>
#if !SILVERLIGHT
        [DefaultValue( true )]
        public bool IsRegistryHiveCurrentUser
        {
            get { return LogNutRegistryService.The.IsSpecificToUser; }
            set { LogNutRegistryService.The.IsSpecificToUser = value; }
        }
#endif

        /// <summary>
        /// Set to access the registry values for LogNut from that section of the Registry that is specific to the current user
        /// (ie, HKEY_CURRENT_USER) versus accessible across all users on the local box
        /// (ie, HKEY_LOCAL_MACHINE).
        /// </summary>
        /// <returns>a reference to this LogConfig object, such that further method calls may be chained</returns>
        /// <remarks>
        /// If you set this, you need to do it before the first Registry access.
        /// There can be security issues related to changing values in HKEY_LOCAL_MACHINE, thus HKEY_CURRENT_USER may be the desired choice.
        /// Calling this method is the same as setting the IsRegistryHiveCurrentUser property to true.
        /// </remarks>
        public LogConfig UseRegistryHiveForCurrentUser()
        {
            IsRegistryHiveCurrentUser = true;
            return this;
        }

        /// <summary>
        /// Set to access the registry values for LogNut from that section of the Registry that is common to the local machine
        /// (ie, HKEY_LOCAL_MACHINE) as opposed to that which applies only to the current user on the local box
        /// (ie, HKEY_CURRENT_USER).
        /// </summary>
        /// <returns>a reference to this LogConfig object, such that further method calls may be chained</returns>
        /// <remarks>
        /// If you set this, you need to do it before the first Registry access.
        /// There can be security issues related to changing values in HKEY_LOCAL_MACHINE, thus HKEY_CURRENT_USER may be the desired choice.
        /// Calling this method is the same as setting the IsRegistryHiveCurrentUser property to false.
        /// </remarks>
        public LogConfig UseRegistryHiveLocalMachine()
        {
            IsRegistryHiveCurrentUser = false;
            return this;
        }
        #endregion

        #region IsUsingRegistry
        /// <summary>
        /// Get or set whether to use the Windows Registry as a source of configuration settings.
        /// Default is false.
        /// </summary>
        [DefaultValue( false )]
        public bool IsUsingRegistry
        {
            get { return _isUsingRegistry; }
            set { _isUsingRegistry = value; }
        }
        #endregion
#endif
        #endregion Windows Registry related properties
        ...
        {
            _lowestLevelThatIsEnabled = default( LogOutputLevel );
            return this;
        }
        #endregion
        ...
        /// <summary>
        /// Given a LogOutputLevel, enable the levels from Fatal down to the given level (inclusive),
        /// and disable the remaining, lower levels.
        /// </summary>
        /// <param name="level">The LogOutputLevel to enable down to, from the highest priority down to this one</param>
        /// <returns>a reference to this LogConfig object so that methods may be chained together</returns>
        /// <remarks>
        /// Given a LogOutputLevel, enable the levels for this logger from <c>Fatal</c> down to the given level (inclusive),
        /// descending by order of 'severity', and disable the remaining, lower levels.
        /// Ie <c>Info</c> -> enable all levels;
        /// <c>Debug</c> -> enable <c>Debug</c>, <c>Warn</c>, <c>Error</c>, <c>Fatal</c>;
        /// <c>Warn</c> -> <c>Warn</c>, <c>Error</c>, <c>Fatal</c>;
        /// <c>Error</c> -> <c>Error</c>, <c>Fatal</c>,
        /// and <c>Fatal</c> -> enable only <c>Fatal</c>.
        /// </remarks>
        public LogConfig EnableLevelsDownTo( LogOutputLevel level )
        {
        ...
        /// </remarks>
        [DefaultValue( LogOutputLevel.Trace )]
        public LogOutputLevel LowestLevelThatIsEnabled
        {
            get { return _lowestLevelThatIsEnabled; }
            set
            {
                _lowestLevelThatIsEnabled = value;
                if (_lowestLevelThatIsEnabled != LogOutputLevel.Trace)
                {
                    LogManager.IsCleared = false;
                }
            }
        ...
        {
            get { return _lowestLevelThatIsEnabled == LogOutputLevel.Trace; }
        }
        #endregion
        ...
        {
            get { return _lowestLevelThatIsEnabled <= LogOutputLevel.Debug; }
        }
        #endregion

        #region IsInfoEnabled
        /// <summary>
        /// Get whether Info-level output is enabled, for all LogNut loggers of this LogManager. Default is true.
        /// </summary>
        public bool IsInfoEnabled
        {
            get { return _lowestLevelThatIsEnabled <= LogOutputLevel.Info; }
        }
        #endregion

        #region IsWarnEnabled
        /// <summary>
        /// Get whether Warn-level output is enabled.
        /// </summary>
        public bool IsWarnEnabled
        {
            get { return _lowestLevelThatIsEnabled <= LogOutputLevel.Warn; }
        }
        #endregion
        ...
        {
            get { return _lowestLevelThatIsEnabled <= LogOutputLevel.Error; }
        }
        #endregion
        ...
        public string FileOutputFolder
        {
            ...
            set
            {
                ...
                {
                    // "ED:" is our special prefix that indicates we want to put the logs onto the drive of the running subject-program's executable.
                    SetFileOutputToFolderOnDriveOfExecutable( value, false );
                }
#endif
                else
                {
        ...
        #region SetFileOutputFolderToThatOfSubjectProgram
#if !NETFX_CORE
        /// <summary>
        /// Set the folder that loggers will write the log file output to, to be the location of the subject-program's execubale.
        /// </summary>
        /// <returns>a reference to this LogConfig object, such that further method calls may be chained</returns>
        /// <remarks>
        /// Unless the output-folder is explicitly set, it defaults to the value of the property <see cref="LogManager.FileOutputFolder_DefaultValue"/>
        /// which on Windows is a "Logs" folder within the Documents folder.
        /// 
        /// You can control what this considers to be the folder of the subject-program by assigning a value to <c>ExplicitlySetFolderOfSubjectProgram</c>,
        /// which is intended for testing purposes.
        /// </remarks>
        public LogConfig SetFileOutputFolderToThatOfSubjectProgram()
        {
            FileOutputFolder = NutUtil.GetProgramExecutionDirectory();
            return this;
        }
#endif
        #endregion

        #region SetFileOutputToFolderOnDriveOfExecutable
#if !NETFX_CORE
        /// <summary>
        /// Set the file-output to go the a folder-path that is denoted by the given directory, but on the disk-drive
        /// that the subject-program is currently executing on.
        /// </summary>
        /// <param name="directoryToPutLogFilesInto">the folder to place the logging output files into (the drive-spec on this will be ignored)</param>
        /// <param name="ensureExists">set this to true to ensure that folder exists or can be created</param>
        /// <returns>a reference to this LogConfig object, such that further method calls may be chained</returns>
        /// <exception cref="LoggingException">The drive-letter of the subject-program's executable must be ascertainable.</exception>
        /// <remarks>
        /// Important Note: If the specified directory on that disk-drive does not already exist, this attempts to create it.
        /// If it is unable to create the directory, then it shifts to the same folder but on the C: drive
        /// assuming that to be more generally accessible to the user.
        /// </remarks>
        public LogConfig SetFileOutputToFolderOnDriveOfExecutable( string directoryToPutLogFilesInto, bool ensureExists )
        {
            string folderOfExecutable = NutUtil.GetProgramExecutionDirectory();
            string drive, folder;
            bool ok = FileStringLib.GetDriveAndDirectory(folderOfExecutable, out drive, out folder);
            if (!ok)
            {
                throw new LoggingException( @"within LogConfig.SetFileOutputToFolderOnDriveOfExecutable(""{0}"", {1}), Unable to determine drive and folder; folderOfExecutable = {2}", directoryToPutLogFilesInto, ensureExists, folderOfExecutable );
            }
            string folderToUse;
            if (directoryToPutLogFilesInto.StartsWith( "ED:" ))
            {
                folderToUse = directoryToPutLogFilesInto.Substring( 3 );
            }
            else
            {
                folderToUse = directoryToPutLogFilesInto;
            }
            string pathToUse = FileStringLib.CombineDriveAndFolder(drive, folderToUse);
            if (ensureExists)
            {
                //CBL And add that we write what goes on here, to the log also.
                // If we cannot create the output folder on that drive,
                // the fallback choice will be the C: drive instead, since that is usually under the user's control.
                string secondChoice = FileStringLib.CombineDriveAndFolder("C:", folderToUse);
                // CBL This violates the reason for the IFilesystem!
                IAccessibilityEvaluator filesystem = new AccessibilityEvaluator();
                string reason, descriptionOfPathChosen;
                FileOutputFolder = NutUtil.FindWorkingFileOutputFolder( filesystem, pathToUse, secondChoice, out descriptionOfPathChosen, out reason );
                // If 'reason' is not null then a different path had to be selected.  CBL Make that part of it's comment!
                if (reason != null)
                {
                    NutUtil.WriteToConsole( descriptionOfPathChosen + ". Using " + FileOutputFolder + " instead." );
                }
            }
            else
            {
                FileOutputFolder = pathToUse;
            }
            return this;
        }
#endif
            ...
            get
            {
                if (_fileOutputArchiveFolder_FullPath == null)
                {
                    if (StringLib.HasSomething( _fileOutputArchiveFolder ))
                    {
                        ...
                        {
#if !NETFX_CORE
                            // it does start with a drive + root
                            // "ED:" is our special prefix that indicates we want to put the logs onto the drive of the running subject-program's executable.
                            if (drive.Equals( "ED" ))
                            {
                                string driveOfExecutable, folderOfExecutable;
                                string pathOfExecutable = NutUtil.GetProgramExecutionDirectory();
                                FileStringLib.GetDriveAndDirectory( pathOfExecutable, out driveOfExecutable, out folderOfExecutable );
                                _fileOutputArchiveFolder_FullPath = FileStringLib.CombineDriveAndFolder( driveOfExecutable, folder );
                            }
                            else
#endif
                            {
        ...
        {
            get { return FileLib.IsFileOutputFilenameUsingDate; }
            set { FileLib.IsFileOutputFilenameUsingDate = value; }
        }
        #endregion
        ...
        public LogConfig SetFileOutputFilenameToIncludeDateTime( string originalFilenameWithoutExtension, string extension )
        {
            ...
            FileOutputFilename = FileLib.CreateLogOutputFilenameWithDate( originalFilenameWithoutExtension: originalFilenameWithoutExtension, extension: extension );
            return this;
        }
        #endregion
        ...
        public void SetFileOutputFilenameForCurrentTime()
        {
            ...
            {
                FileOutputFilename = FileLib.RecreateFileOutputFilename();
            }
        }
        #endregion
        ...
        /// <summary>
        /// Get or set whether to include the log-level in the prefix for each log record only when that level is a Warning, Error, or Fatal.
        ...
        /// <summary>
        /// Enable the showing of the Level for each log-record only when that level is at LogOutputLevel.Warning or higher,
        /// that is - for Warning, Error, and Fatal.
        /// </summary>
        /// <returns>a reference to this LogConfig object so that methods may be chained together</returns>
        /// <remarks>
        /// The intent of this is to permit further uncluttering of the log-file for non-important log traces.
        /// It may be desired to see "Warning", "Error", or "Fatal" but not to distinguish the level of output that is less critical than that. 
        /// </remarks>
        ...
        /// <summary>
        /// Get or set whether to display the timestamp for each log-record with include fractions of a second,
        ...
        public void SetToDefaults()
        {
            ...
            LowestLevelThatIsEnabled = default( LogOutputLevel );
            IsLoggingEnabled = true;
            _fileOutputFilename = null;
            _fileOutputFolder = null;
            ...
            FileLib.IsFileOutputFilenameUsingDate = false;
        ...
        public override string ToString()
        {
            ...
            if (LowestLevelThatIsEnabled != default( LogOutputLevel ))
            {
        ...
        /// <summary>
        /// This denotes lowest-level LogOutputLevel that is currently enabled. The default is LogOutputLevel.Trace, which means all levels are enabled.
        /// </summary>
        private LogOutputLevel _lowestLevelThatIsEnabled;
        ...
        /// <summary>
        /// This dictates whether to take our settings from the Windows Registry. Default is false.
        /// </summary>
        private static bool _isUsingRegistry;
#endif
        ...
        /// <summary>
        /// This dictates whether to include the logging level (the LogOutputLevel) in the prefix for each log record.
        /// Default is true.
        /// </summary>
        private bool _isToShowLevel = true;

        /// <summary>
        /// When this is true, the log-level is included within the prefix only when it is Warn or Error.
