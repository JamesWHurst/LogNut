...
namespace Hurst.LogNut
{
    ...
    public class LoggingFaultEventArgs : EventArgs
    {
        ...
        public LoggingFaultEventArgs( string userSummaryMessage, string reason, Exception exception, LogRecord record )
        {
        ...
        public string GetDetailedDeveloperMessage()
        {
            ...
            if (Record != null && Record.Level >= LogOutputLevel.Info && StringLib.HasSomething( Record.Message ))
            {
                if (Reason != null)
                {
                    sb.AppendLine();
