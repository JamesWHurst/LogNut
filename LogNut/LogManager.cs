...
namespace Hurst.LogNut
{
    ...
    public static partial class LogManager
    {
        ...
        /// <summary>
        /// This event is raised whenever any of the LogException methods are called.
        ...
        /// Yes - if you hook both events, LogException will throw both events.
        /// </summary>
        ...
        public static void ConvertEventLogEntriesToLogFiles( string source, string logName )
        {
            //CBL Tinkering
            using (EventLog eventLog = new EventLog( logName: logName ))
            {
...
#if !NETFX_CORE
            //CBL But do we want to check the Registry first?
            if (Config.IsUsingRegistry)
            {
                // Since this is the main entry-point into using the LogNut facility,
                // this is a logical place to ensure the Registry
                // contains the key paths we are going to require.
                if (!_isRegistryCheckDone)
                {
                    _isRegistryCheckDone = true;
#if !SILVERLIGHT
                    //CBL Do we actually want to use the type LogManager for this? Seems incorrect.
                    LogNutRegistryService.The.EnsureLogNutRegistryTreeExists( Config.GetSubjectProgramName( typeof( LogManager ) ), null );
#endif
                }
            }
#endif
        ...
        /// 
        /// If no C# delcaring-type (class) is found, then the default logger is returned.
        /// 
        ...
        /// using the default logger.
        /// This is just a shortcut for <c>LogManager.GetLogger().LogFatal</c> .
        /// </summary>
        /// <param name="textToLog">the string that represents the message to log</param>
        public static void LogFatal( string textToLog )
        {
            var logger = GetLogger();
            logger.LogFatal( textToLog );
        }
        ...
        /// This is a shortcut for LogManager.GetLogger().LogException.
        /// </summary>
        ...
        {
            var logger = GetLogger();
            logger.LogException( exception, String.Format( format, args ) );
        }
        #endregion
        ...
        public static string FileOutputFolder_DefaultValue
        {
            get
            {
                ...
                return LognutLib.GetDefaultFileOutputDirectory();
            }
        }
        #endregion
        ...
        #region ClearVisualStudioOutputWindow
        /// <summary>
        /// Clear the Output window of Visual Studio 2010, if running in the debugger.
        /// This is simply a minor convenience, since that window is often busy with unhelpful outputs before the program code even starts executing.
        /// I don't know why I added this. Just for kicks I guess.
        /// </summary>
        public static void ClearVisualStudioOutputWindow()
        {
            try
            {
                VisualStudioLib.ClearOutputWindow();
            }
            catch (Exception x)
            {
                NutUtil.WriteToConsole( "in LogManager.ClearOutputWindow", x );
            }
        }
        #endregion
        ...
        public static bool IsInDesignMode
        {
            get
            {
                if (_config == null || _config.SubjectProgram == null)
                {
                    return false;
                }
                return _config.SubjectProgram.IsInDesignMode;
            }
        }
        #endregion
        ...
        public static void Clear()
        {
            ...
            if (!IsCleared)
            {
                ...
                _config = null;
                NutUtil.GetterOfExecutionFolder = null;
                NutUtil.ValueToUseForSubjectProgramFolder = null;
        ...
        internal static void QueueOrSendToLog( Logger logger, LogRecord logRecord, bool isToSuppressTraceOutput )
        {
            ...
            try
            {
                ...
                if (!(IsTesting && TestFacility.IsSimulatingOutput))
                {
        ...
        public static void WriteToSvcLogFile( string message )
        {
            ...
            lock (FileLockObject)
            {
                ...
                {
                    string filename = FileLib.CreateLogOutputFilenameWithDate(originalFilenameWithoutExtension: LogManagerSvcLogBaseFilename, extension: "txt");
                        ...
                        else
                        {
                            ...
                            catch (Exception x)
                            {
                                ...
                                if (x is Win32Exception && x.Message.Contains( "is being used by another process" ))
                                {
                                    ...
                                    string newPath = ZPath.Combine(destinationDir, newFilename);
        ...
        /// <summary>
        /// Within the standard log-record prefix, this is the character that separates the LogOutputLevel from the design-view indicator.
        /// </summary>
...
#if !NETFX_CORE
        /// <summary>
        /// This flag is set true once we have done our check of the Windows Registry, to avoid doing it with every call to GetLogger.
        /// </summary>
        private static bool _isRegistryCheckDone;
#endif
